...
import com.dm.study.cloud.feign.SecurityToUserService;
...
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	@Resource
	SecurityToUserService toUserService;

	@Override
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		System.out.println(new BCryptPasswordEncoder().encode("123456"));
		SysUser user = toUserService.queryUserByUsername(username);
		if (user == null) {
	...
	public UserDetails createLoginUser(SysUser user) {
		...
		// 1.生成token
		loginUser.setToken(jwtTokenUtil.generateToken(user.getUsername()));
		// 2.处理权限
